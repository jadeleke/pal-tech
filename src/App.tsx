...
function App() {
  ...
  return <div className="app-shell"><div ref={siteRef} className="site-frame" aria-hidden={showWelcome||undefined}>
    ...
    <footer><div className="footer-brand"><span className="foundation-logo-crop footer-logo"><img className="foundation-logo" src="/play-learn-foundation-logo.png" alt="Play & Learn Foundation"/></span><span><b>Play & Learn Foundation</b><small>Learn • Practice • Play • Build</small></span></div><div className="footer-links"><button onClick={()=>go('about')}>About & Legacy</button><button onClick={()=>go('facilitator')}>Facilitator Corner</button><span>PAL Tech v1.0.0</span></div></footer></div>
...
function HomePage({progress,overall,openModule,go}:{progress:ProgressState;overall:number;openModule:(m:LearningModule)=>void;go:(v:View)=>void}) {
  ...
  return <>
    ...
    <section className="content-section"><div className="section-heading"><div><span className="eyebrow">EXPLORE THE LABS</span><h2>Choose your next adventure</h2><p>Eight labs. 32 short lessons. Pick one that sparks your curiosity.</p></div><button className="text-button arrow" onClick={()=>go('learn')}>See all labs <ArrowRight size={18}/></button></div><div className="module-grid">{modules.map((module,index)=><ModuleCard key={module.id} module={module} progress={progress} index={index} open={()=>openModule(module)}/>)}</div></section>
  ...
  return <section className="lesson-page"><div className="lesson-head" style={{'--accent':module.colour} as React.CSSProperties}><button className="back" onClick={back}><ArrowLeft size={18}/> All labs</button><div className="lesson-title"><div className="module-icon large">{module.icon}</div><div><span className="eyebrow">{module.title.toUpperCase()}</span><h1>{lesson.title}</h1></div></div><div className="lesson-meter"><span>Lesson {active+1} of {module.lessons.length}</span><div className="progress-track"><i style={{width:`${((active+1)/module.lessons.length)*100}%`}}/></div></div><p className="lesson-swipe-hint">Swipe the lesson cards below to see all four →</p></div>
    <div className="lesson-layout"><aside className="lesson-nav" aria-label="Lessons">{module.lessons.map((l,i)=><button className={`${i===active?'active':''} ${progress.completedLessons.includes(l.id)?'done':''}`} onClick={()=>{setActive(i);setChoice('');setChecked(false)}} key={l.id}><span>{progress.completedLessons.includes(l.id)?<Check size={16}/>:i+1}</span><div><small>{l.eyebrow}</small>{l.title}</div></button>)}</aside>
      <article className="lesson-content"><span className="eyebrow">BIG IDEA</span><h2>{module.bigIdea}</h2><div className="concept-block"><span className="block-label">LEARN</span><p>{lesson.concept}</p></div><div className="example-block"><div className="example-icon">◎</div><div><span className="block-label">SEE IT IN REAL LIFE</span><p>{lesson.example}</p></div></div>
        ...
        {active===module.lessons.length-1&&done&&<div className="completion"><Award size={34}/><div><h3>Lab lessons complete</h3><p>Try the lab quiz to work toward your {badgeNames[module.id][0]} badge.</p></div><button className="primary" onClick={goQuiz}>Open Quiz Arena</button></div>}
      </article></div>
  </section>
}
...
function QuizPage({progress,update}:{progress:ProgressState;update:(p:ProgressState)=>void}) {
  ...
  const start=(id:string)=>{const pool=id==='mixed'?allQuestions:modules.find(m=>m.id===id)!.questions; setTopic(id);setQuestions([...pool].sort(()=>0.5-Math.random()).slice(0,5));setIndex(0);setScore(0);setChoice('');setChecked(false);setHadMistake(false);setFinished(false)};
  if(!questions.length)return <section className="page content-section"><div className="page-intro"><span className="eyebrow">QUIZ ARENA</span><h1>Test what you know.</h1><p>You can retry every answer. Your first try sets your score.</p></div><div className="quiz-picker"><button className="featured" onClick={()=>start('mixed')}><span>⚡</span><div><b>Mixed Tech Challenge</b><small>5 questions from across every lab</small></div><ArrowRight/></button>{modules.map(m=><button onClick={()=>start(m.id)} key={m.id}><span>{m.icon}</span><div><b>{m.shortTitle} quiz</b><small>5 questions • best first-try score saved</small></div><ChevronRight/></button>)}</div></section>;
